# cmdfilter release checklist

This document describes a practical release flow for `cmdfilter`.
...
python -m ruff check cmdfilter
python -m mypy cmdfilter --config-file mypy.ini
python -m pytest -q
```
...
python -m pip install -U pip
python -m pip install -i https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple cmdfilter==0.1.0
python -c "import cmdfilter; print(cmdfilter.__name__)"
deactivate
```
